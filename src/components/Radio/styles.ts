...
export const dotPlacement = (isSelected: boolean) => ({ colors }: Theme) => css`
  width: 16px;
  height: 16px;
...
export const placementContainer = css`
  debug: Radio-Placement;
  position: relative;
  display: inline-block;
  text-align: center;
  cursor: pointer;
...
export const checkedPlacemetBackground = (checkedState: boolean) => ({ colors }: Theme) => {
  if (checkedState) {
    return css`
